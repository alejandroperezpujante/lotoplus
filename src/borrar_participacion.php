...
<main>
	...
	if ($res->num_rows > 0) {
		echo "<form action='borrar_participacion.php' method='post' enctype='multipart/form-data'>";
		...
		echo "</select>";
		echo "<input type='submit' value='Borrar participación' name='borrar_participacion' id='borrar_participacion'>";
		echo "</form>";
	...
	if (isset($_POST['borrar_participacion'])) {
		$participation_id = $_POST['participations_list'];
		$cnx = db();
		$sql = "DELETE FROM participations WHERE participation_id = $participation_id";
		$res = $cnx->query($sql) or die($cnx->error);
		if ($res) {
			echo '<h3>Participación borrada</h3>';
			header('Refresh: 2; url=borrar_participacion.php');
		} else {
			echo '<h3>No se ha podido borrar la participación</h3>';
			header('Refresh: 2; url=borrar_participacion.php');
		}
		$res->free();
		$cnx->close();
